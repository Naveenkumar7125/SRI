...
// // Import video assets
// // import cam1 from '../assets/cam1.mp4';
// const cam1 = "https://res.cloudinary.com/dprwjya79/video/upload/v1760464239/Realistic_Drone_Feed_Video_Generation_fj9pgr.mp4"
// // import cam2 from '../assets/cam2.mp4';
// const cam2 = "https://res.cloudinary.com/dprwjya79/video/upload/v1760502657/Drone_View_of_Road_Accident_Scene_qzgpun.mp4"
// import cam3 from '../assets/cam3.mp4';
// // import cam4 from '../assets/cam4.mp4';
// // import cam5 from '../assets/cam5.mp4';
// // import cam6 from '../assets/cam6.mp4';
...
// Video assets for each feed
const videoAssets = [
  "https://res.cloudinary.com/dprwjya79/video/upload/v1760464239/Realistic_Drone_Feed_Video_Generation_fj9pgr.mp4", // Drone 1
  "https://res.cloudinary.com/dprwjya79/video/upload/v1760502657/Drone_View_of_Road_Accident_Scene_qzgpun.mp4", // Drone 2
  cam3, // Fixed Cam 1
  "https://res.cloudinary.com/dprwjya79/video/upload/v1760504715/Realistic_Surveillance_Video_Generation_eu1gdi.mp4", // Fixed Cam 2
  cam2// Robot Patrol
];
...
export default function LiveFeedsPage() {
  ...
  // Assign a unique video to each feed
  useEffect(() => {
    const initialFeeds = generateVideoFeeds();
    const feedsWithVideos = initialFeeds.map((feed, index) => ({
      ...feed,
      heatmapUrl: createHeatmapDataUrl(),
      videoUrl: videoAssets[index] || videoAssets[0], // unique video for each feed
    }));
    setFeeds(feedsWithVideos);
  ...
  // Play/pause videos based on view mode
  useEffect(() => {
    Object.values(videoRefs.current).forEach((video) => {
      if (video) {
        if (viewMode === 'live') {
  ...
  useEffect(() => {
    if (isModalOpen && modalVideoRef.current) {
      modalVideoRef.current.play().catch(console.log);
    }
  }, [isModalOpen]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex space-x-2">
          <button
            onClick={() => setViewMode('live')}
            className={`px-4 py-2 rounded-lg font-medium transition-all ${
              viewMode === 'live' ? 'bg-neonBlue text-navy neon-glow' : 'bg-steel text-textPrimary hover:bg-steel/70'
            }`}
          >
            Live View
          </button>
          <button
            onClick={() => setViewMode('heatmap')}
            className={`px-4 py-2 rounded-lg font-medium transition-all ${
              viewMode === 'heatmap' ? 'bg-neonBlue text-navy neon-glow' : 'bg-steel text-textPrimary hover:bg-steel/70'
            }`}
          >
            Heatmap View
          </button>
          ...
          >
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs font-semibold ${
                    feed.status === 'live' ? 'bg-successGreen/20 text-successGreen' : 'bg-alertRed/20 text-alertRed'
                  }`}
                >
                  {feed.resolution}
                </span>
